...
// Common subject names across Indian boards — used to gate the riskiest
// single-number/grade-only patterns so OCR noise (e.g. a stray misread
// border character that happens to look like "WORD 5") doesn't get accepted
// as a fake subject when there's no three-number line to anchor it.
const KNOWN_SUBJECT_KEYWORDS = [
  'ENGLISH', 'HINDI', 'SANSKRIT', 'URDU', 'MATHEMATICS', 'MATHS', 'SCIENCE',
  'SOCIAL SCIENCE', 'PHYSICS', 'CHEMISTRY', 'BIOLOGY', 'COMPUTER',
  'INFORMATICS', 'PHYSICAL EDUCATION', 'ECONOMICS', 'ECONOMIC APPLICATIONS',
  'COMMERCIAL APPLICATIONS', 'ACCOUNTANCY', 'BUSINESS STUDIES', 'HISTORY',
  'GEOGRAPHY', 'CIVICS', 'POLITICAL SCIENCE', 'PSYCHOLOGY', 'SOCIOLOGY',
  'PAINTING', 'HOME SCIENCE', 'GUJARATI', 'MARATHI', 'TAMIL', 'TELUGU',
  'KANNADA', 'MALAYALAM', 'BENGALI', 'PUNJABI', 'ORIYA', 'ODIA',
  'ENVIRONMENTAL SCIENCE', 'WORK EDUCATION', 'ART', 'MUSIC', 'DANCE', 'YOGA',
  'GENERAL KNOWLEDGE', 'MORAL SCIENCE', 'VALUE EDUCATION', 'DRAWING', 'CRAFT',
  'AGRICULTURE', 'LEGAL STUDIES', 'ENTREPRENEURSHIP', 'MULTIMEDIA',
  'WEB APPLICATION', 'LITERATURE', 'LANGUAGE', 'STATISTICS', 'ELECTIVE',
];
const isKnownSubjectName = (name) => {
  const upper = name.toUpperCase();
  return KNOWN_SUBJECT_KEYWORDS.some((kw) => upper.includes(kw) || kw.includes(upper));
};
...
const extractCBSESubjects = (lines, fullText) => {
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      if (name && name.length >= 2 && !isNonSubject(name) && !isDuplicate(subjects, name) &&
          !isNaN(marks) && marks >= 0 && marks <= 100 && isKnownSubjectName(name)) {
    ...
    if (match) {
      ...
      if (name && name.length >= 2 && !isNonSubject(name) && !isDuplicate(subjects, name) && isKnownSubjectName(name)) {
...
// ICSE 6th/elective subjects that are always printed as their own standalone
// line (never as a sub-component of another subject group) — used to recover
// them even when OCR drops the spelled-out "words" column that normally
// distinguishes a group line from a sub-component line.
const ICSE_STANDALONE_ELECTIVES = [
  'ECONOMIC APPLICATIONS', 'COMMERCIAL APPLICATIONS', 'COMPUTER APPLICATIONS',
  'COMPUTER SCIENCE', 'ENVIRONMENTAL SCIENCE', 'ENVIRONMENTAL APPLICATIONS',
  'PHYSICAL EDUCATION', 'HOME SCIENCE', 'ART', 'MUSIC', 'YOGA',
];
const isICSEStandaloneElective = (name) => {
  const upper = name.toUpperCase();
  return ICSE_STANDALONE_ELECTIVES.some((kw) => upper.includes(kw) || kw.includes(upper));
};
...
const extractICSESubjects = (lines, fullText) => {
  ...
  if (groupSubjects.length > 0) {
    // Recover standalone electives (e.g. "Economic Applications") that OCR
    // printed without the spelled-out words column, so they didn't qualify
    // as a group line — but are still clearly not a sub-component of
    // anything else on an ICSE marksheet.
    for (const extra of componentSubjects) {
      if (isICSEStandaloneElective(extra.name) && !isDuplicate(groupSubjects, extra.name)) {
        groupSubjects.push(extra);
      }
    }
...
const cleanSubjectName = (raw) => {
  ...
  return raw
    ...
    .replace(/[\s\-\/\.]+$/, '') // strip trailing stray punctuation from OCR noise
    .trim();
};
