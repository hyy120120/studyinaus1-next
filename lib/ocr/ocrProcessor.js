...
const detectFormat = (text, origin) => {
  ...
  // French Bac (/20)
  if (origin === 'FRANCE' || /BACCALAURÉAT|\/20/.test(upper)) return 'FRENCH';
...
const extractStudentInfo = (text, origin) => {
  ...
  const namePatterns = [
    /candidate['s\s]*name\s*[:\-.]?\s*([A-Z][A-Z\s]{3,60})/i,
    /student['s\s]*name\s*[:\-.]?\s*([A-Z][A-Z\s]{3,60})/i,
    /name\s+of\s+(?:the\s+)?student\s*[:\-.]?\s*([A-Z][A-Z\s]{3,60})/i,
    /name\s*[:\-]\s*([A-Z][A-Z\s]{3,50})/i,
    // After seat number (Indian boards)
    /[A-Z]\s*\d{5,8}\s+([A-Z][A-Z\s]{5,50})\b/,
    // After "Mr/Ms/Mrs"
    /(?:Mr|Ms|Mrs|Dr|Prof)\.?\s+([A-Z][A-Za-z\s]{3,50})/,
  ];
  for (const p of namePatterns) {
    const m = text.match(p);
    if (m) {
      // Capture group can cross a newline since \s matches it — keep only the first line
  ...
  const fatherMatch = text.match(
    /(?:father['s]*\s*name|father)\s*[:\-]\s*([A-Za-z\s]{3,50})/i
  );
  if (fatherMatch) info.fatherName = fatherMatch[1].trim();

  const motherMatch = text.match(
    /(?:mother['s]*\s*name|mother)\s*[:\-]\s*([A-Za-z\s]{3,50})/i
  );
...
const extractCBSESubjects = (lines, fullText) => {
  ...
  for (const line of lines) {
    ...
    // (a) subject code + name + theory + practical/internal + total [+ grade]
    //     e.g. "041 MATHEMATICS 80 20 100 A1"
    let match = t.match(
      /^(?:\d{2,3}[_\-.\s]+)?([A-Za-z][A-Za-z\s&\-\.\/\(\)]{2,45}?)\s+(\d{1,3})\s+(\d{1,3})\s+(\d{1,3})\s*(?:(A1|A2|B1|B2|C1|C2|D1|D2|E1|E2)\b)?/i
    );
    if (match) {
      const name = cleanSubjectName(match[1]);
      ...
      if (name && name.length >= 2 && !isNonSubject(name) && !isDuplicate(subjects, name) &&
        ...
        subjects.push({
          ...
          distinctionLabel : getDistinctionLabel(dist),
          isPassed         : pct >= 33,
          cbseGrade        : match[5] ? match[5].toUpperCase() : '',
        });
        continue;
      }
    }
...
const extractICSESubjects = (lines, fullText) => {
  ...
  for (const line of lines) {
    ...
    // (a) "Subject  Internal  External  Total"  e.g. "ENGLISH 20 65 85"
    let match = t.match(
    ...
    if (match) {
      ...
      if (name && name.length >= 2 && !isNonSubject(name) && !isDuplicate(subjects, name) &&
        ...
        subjects.push({
          ...
          maxMarks         : 100,
          obtainedMarks    : total,
          percentage       : parseFloat(total.toFixed(2)),
          distinction      : dist,
          distinctionLabel : getDistinctionLabel(dist),
          isPassed         : total >= 33,
        });
    ...
    // (b) "Subject  Marks"  single value out of 100 — the common ICSE line format
    match = t.match(/^([A-Za-z][A-Za-z\s&\-\.\/\(\)]{2,45}?)\s{1,10}(\d{1,3})\s*$/);
    if (match) {
      const name  = cleanSubjectName(match[1]);
      const marks = parseInt(match[2], 10);
      if (name && name.length >= 2 && !isNonSubject(name) && !isDuplicate(subjects, name) &&
  ...
  // ICSE marksheets often also carry a normal "name  obtained  max" line for
  // a couple of subjects (e.g. group II/III electives) — merge those in too.
  if (subjects.length > 0) {
    for (const line of lines) {
      const extra = parseStandardLine(line);
      if (extra && !isDuplicate(subjects, extra.name)) subjects.push(extra);
    }
  }
...
const extractTotals = (text, subjects, format) => {
  ...
  if (!result.obtained) {
    const fracMatch = text.match(/\b(\d{2,4})\s*\/\s*(\d{3,4})\b/);
    if (fracMatch) {
