...
export function downloadVisaReportPdf(record) {
    ...
    const snapFields = [
        ...
        ["Grade %", form.grade_percentage],
        ["Year of completion", form.year_of_completion],
        ["Gap years", form.gap_years],
        ["Work experience (yrs)", form.work_experience_years],
        ["Work relevant", form.work_relevant_to_course ? "Yes" : "No"],
        ["English test", `${form.english_test || ""} ${form.english_score ?? ""}`],
        ["Intended course", form.intended_course],
        ["Intended university", form.intended_university || "—"],
        ["Intake year", form.intake_year],
        ["Sponsor", form.sponsor_relationship],
        ["Family income (INR)", fmtNumber(form.annual_family_income_inr)],
        ["Income proof", form.income_proof_available ? "Yes" : "No"],
        ["Liquid funds (INR)", fmtNumber(form.liquid_funds_inr)],
        ["Loan sanctioned (INR)", fmtNumber(form.loan_sanctioned_inr)],
        ["Property/assets (INR)", fmtNumber(form.property_assets_inr)],
        ["Previous refusal", form.previous_visa_refusal ? "Yes" : "No"],
        ["Refusal country", form.refusal_country || "—"],
        ["Prior AU visa", form.prior_australia_visa ? "Yes" : "No"],
    ].map(([k, v]) => [k, String(v ?? "—")]);
