...
export async function downloadVisaReportPdf(record) {
    ...
    const section = (title, items, color, emptyMessage) => {
        ...
        if (!items.length) {
            doc.setTextColor(MUTED);
            const lines = doc.splitTextToSize(emptyMessage || "—", contentWidth);
    ...
    if (record.ai_assessment) {
        section("Counselor assessment", [record.ai_assessment], PRIMARY);
    }
    section("Strengths", record.strengths || [], "#2D6C4A", "Build foundational strengths in English and academic readiness first.");
    section("Areas to Improve", record.weaknesses || [], "#DD8226", "No critical weaknesses detected — keep documentation airtight.");
    section("Recommended Next Actions", record.recommendations || [], PRIMARY, "You’re tracking well. Reach out to schedule a final review with a counselor.");
    ...
    const snapFields = [
        ...
        ["Education loan", form.education_loan_required ? "Yes" : "No"],
        ...(form.education_loan_required ? [["Education loan amount (INR)", fmtNumber(form.loan_amount_inr)]] : []),
