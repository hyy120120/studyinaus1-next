...
function academicPoints(form) {
    ...
    const highest = applicable
        .slice()
        .sort((a, b) => (a.passout_year || a.end_year || 0) - (b.passout_year || b.end_year || 0))
        .pop();
...
function workPoints(form) {
    let pts = 0;
    const records = Array.isArray(form.employment_records) ? form.employment_records : [];
    const work1 = records.some((record) => ["Employed", "Self-employed"].includes(record.status)) || ["Employed", "Self-employed"].includes(form.work1_status);
    const work2 = records.filter((record) => ["Employed", "Self-employed"].includes(record.status)).length > 1 || ["Employed", "Self-employed"].includes(form.work2_status);

    if (work1 || work2) pts += 3;
    if (form.work_relevant_to_course) pts += 3;
    if (records.some((record) => record.itr_filed) || form.work1_itr_filed || form.work2_itr_filed) pts += 2;
...
function financialPoints(form) {
    ...
    const proofCount = sponsors.reduce((sum, s) => sum + (Array.isArray(s?.docs) ? s.docs.filter((doc) => doc?.status === "yes").length : 0), 0);
    const timelyItrCount = sponsors.filter((s) => s?.itr_timely).length;
    const threeYearItrCount = sponsors.filter((s) => s?.itr_3yr).length;
    ...
    if (timelyItrCount > 0) pts += 1;
    if (threeYearItrCount > 0) pts += 1;
...
export function computeScore(form) {
    ...
    const summary =
        ...
        `history, course continuity, and family/dependant factors. This is a guidance estimate; the final outcome ` +
