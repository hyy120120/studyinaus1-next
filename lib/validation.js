...
export function validateCalculatorStep(step, form) {
    ...
    if (step === 0) {
        ...
        if (form.has_passport) {
            ...
            // Passports are issued for a maximum of 10 years, so an expiry
            // year beyond (current year + 10) can never be correct.
            const maxExpiryYear = now.getFullYear() + 10;
            ...
            if (!errors.passport_expiry_date && form.passport_expiry_date > `${maxExpiryYear}-12-31`) {
                errors.passport_expiry_date = `Passports are valid for a maximum of 10 years — expiry cannot be beyond ${maxExpiryYear}.`;
            }
