...
import { educationCompletionYear, educationTimeline, isSchoolQualification } from "@/lib/educationTimeline";
import { validateEnglishExamDate } from "@/lib/englishExamTimeline";
...
export function isTenDigitPhone(value) {
    if (!/^\d{10}$/.test(String(value || ""))) return "Invalid number. Enter exactly 10 digits.";
    return null;
}
...
export function inRange(value, min, max, label = "Value") {
    // Empty is a missing input, not an out-of-range number — without this
    // guard, "" becomes 0 (Number("") === 0) and produces misleading errors
    // like "10th passout year must be 1995 or later" on an untouched field.
    if (value === "" || value === null || value === undefined) return `Enter ${label}.`;
    const n = Number(value);
    if (Number.isNaN(n)) return `${label} is invalid.`;
    const hasMin = min !== undefined && min !== null;
    const hasMax = max !== undefined && max !== null;
    if (hasMin && n < Number(min)) return hasMax ? `${label} must be between ${min} and ${max}.` : `${label} must be ${min} or later.`;
    if (hasMax && n > Number(max)) return hasMin ? `${label} must be between ${min} and ${max}.` : `${label} must be ${max} or earlier.`;
    return null;
}
...
// --- Dev-only bypass ------------------------------------------------------
// When NEXT_PUBLIC_CALCULATOR_BYPASS_VERIFICATION=true (see
// components/calculator/CalculatorClient.jsx), the calculator skips every
// validation rule so developers can click through all steps with empty data.
// Other forms (course inquiry, counselling booking) are NOT affected.
// Never set the flag in production.
...
export function validateCalculatorStep(step, form) {
    if (DEV_BYPASS_VERIFICATION) return {};
    const errors = {};
    ...
    if (step === 0) {
        ...
        errors.phone = required(form.phone) || isTenDigitPhone(form.phone);
        errors.dob = required(form.dob, "Date of birth is required.");
        if (!errors.dob) errors.dob = inRange(form.age, 12, 65, "Age (from date of birth)");
        ...
        // Passport details are only asked — and therefore only validated —
        // when the applicant says they hold a valid passport. An expired
        // passport counts as "No" (the question asks for a VALID passport).
        if (form.has_passport) {
            const todayStr = new Date().toISOString().slice(0, 10);
            errors.passport_number = required(form.passport_number, "Enter your passport number.") ||
                (/^[A-Za-z0-9]{5,20}$/.test(String(form.passport_number || "").trim()) ? "" : "Passport number must be 5-20 letters or digits (no spaces).");
            errors.passport_place_of_issue = required(form.passport_place_of_issue, "Enter the place where the passport was issued.") || maxLen(form.passport_place_of_issue, 60, "Place of issue");
            errors.passport_issue_date = required(form.passport_issue_date, "Enter the passport issue date.");
            errors.passport_expiry_date = required(form.passport_expiry_date, "Enter the passport expiry date.");
            if (!errors.passport_issue_date && form.passport_issue_date > todayStr) {
                errors.passport_issue_date = "Issue date cannot be in the future.";
            }
            if (!errors.passport_expiry_date && form.passport_expiry_date < todayStr) {
                errors.passport_expiry_date = "This passport has expired — answer No or renew it first.";
            }
            if (!errors.passport_issue_date && !errors.passport_expiry_date && form.passport_expiry_date <= form.passport_issue_date) {
                errors.passport_expiry_date = "Expiry date must be after the issue date.";
            }
        }
        if (!form.privacy_consent) errors.privacy_consent = "Please accept the Privacy Policy to continue.";
        if (!form.terms_consent) errors.terms_consent = "Please accept the Terms of Service to continue.";
    }
    ...
    if (step === 1) {
        ...
        if (applicable.length === 0) {
            ...
            applicable.forEach((l) => {
                ...
                if (!['y10', 'y12'].includes(l.key)) {
                    errors[`edu_${l.key}_stream`] = required(l.stream, `Enter the stream / course for ${l.label}.`);
                }
                const timeline = educationTimeline(form.dob, l.key, l.start_year, previousEndYear);
                if (isSchoolQualification(l.key)) {
                    errors[`edu_${l.key}_passout`] = inRange(l.passout_year, timeline.passout.min, timeline.passout.max, `${l.label} passout year`);
                    if (!errors[`edu_${l.key}_passout`]) previousEndYear = l.passout_year;
                } else {
                    errors[`edu_${l.key}_start`] = inRange(l.start_year, timeline.start.min, timeline.start.max, `${l.label} start year`);
                    errors[`edu_${l.key}_end`] = inRange(l.end_year, timeline.end.min, timeline.end.max, `${l.label} end year`);
                    if (!errors[`edu_${l.key}_end`]) previousEndYear = educationCompletionYear(l);
                }
    ...
    if (step === 2) {
        ...
        if (form.english_test === "Tentative") {
            errors.tentative_exam_date = required(form.tentative_exam_date, "Tentative exam date is required.") || validateEnglishExamDate(form.tentative_exam_date, form.dob, form.english_test, { tentative: true });
        } else if (form.english_test) {
            const range = ENGLISH_SCORE_RANGES[form.english_test] || [0, 999];
            errors.exam_date = required(form.exam_date, "Exam date is required.") || validateEnglishExamDate(form.exam_date, form.dob, form.english_test);
    ...
    if (step === 7) {
        (form.sponsors || []).filter((s) => s.applicable).forEach((s) => {
            (s.docs || []).forEach((doc) => {
                ...
                if (doc.status === "yes" && !doc.file_url) errors[`sponsor_doc_file_${s.id}_${doc.key}`] = `Upload the ${doc.label} file, or mark it as No and add a note.`;
            });
        });
    }
    ...
    if (step === 4) {
        ...
        if (form.work_relevant_to_course === true) {
            errors.work_relevance_explanation = required(form.work_relevance_explanation, "Briefly explain how your work experience relates to the course.") || minLen(form.work_relevance_explanation, 5, "Explanation");
        }
        if (form.work_verification_done === true) {
            errors.work_verification_contact_name = required(form.work_verification_contact_name, "Enter the verification contact name.");
            // Note: plain EMAIL_RE (not isEmail) — employer HR contacts usually
            // sit on company domains, which the "standard provider" rule rejects.
            errors.work_verification_contact_phone = required(form.work_verification_contact_phone, "Enter the verification contact phone number.") || isTenDigitPhone(form.work_verification_contact_phone);
            errors.work_verification_contact_email = required(form.work_verification_contact_email, "Enter the verification contact email.")
                || (EMAIL_RE.test(String(form.work_verification_contact_email || "").trim()) ? null : "Enter a valid email address.");
        }
    }
    ...
    if (step === 5) {
        ...
        if (form.course_in_line_with_previous_education === false) {
            errors.course_change_reason = required(form.course_change_reason, "Explain why you are changing your field of study.") || minLen(form.course_change_reason, 10, "Reason");
        }
    ...
    if (step === 8) {
        ...
        if (form.education_loan_required === true) {
            errors.loan_amount_inr = required(form.loan_amount_inr, "Loan amount is required.") || nonNegative(form.loan_amount_inr, "Loan amount");
        }
    }
...
export function validateBooking(booking) {
    ...
    errors.mobile = required(booking.mobile) || isTenDigitPhone(booking.mobile);
    ...
    if (!booking.privacy_consent) errors.privacy_consent = "Please accept the Privacy Policy to continue.";
    if (!booking.terms_consent) errors.terms_consent = "Please accept the Terms of Service to continue.";
    Object.keys(errors).forEach((k) => { if (!errors[k]) delete errors[k]; });
    return errors;
}
